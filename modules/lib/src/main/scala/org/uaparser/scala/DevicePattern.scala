...
private[scala] case class DevicePattern(
  ...
  def process(agent: String): Option[Device] = {
    ...
    if (!matcher.find()) None
      ...
      val family = familyReplacement.map(r => Util.patternReplacementWithMatcherGroups(r, matcher, agent)).orElse(matcher.groupAt(1))
      val brand = brandReplacement.map(r => Util.patternReplacementWithMatcherGroups(r, matcher, agent)).filterNot(s => s.isEmpty)
      val model = modelReplacement.map(r => Util.patternReplacementWithMatcherGroups(r, matcher, agent)).orElse(matcher.groupAt(1)).filterNot(s => s.isEmpty)
...
private object DevicePattern {
  def fromMap(m: Map[String, String]): Option[DevicePattern] = m.get("regex").map { r =>
    val pattern = m.get("regex_flag").map(_ => Pattern.compile(r, Pattern.CASE_INSENSITIVE)).getOrElse(Pattern.compile(r))
